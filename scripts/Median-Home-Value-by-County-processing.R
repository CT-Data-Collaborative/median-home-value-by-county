...
##################################################################
#
# Processing Script for Median Home Value by County
...
data_long_fips <- merge(data_long, counties, by.x = "FIPS.x", by.y = "FIPS")

data_long_fips$Year <- paste(data_long_fips$Year-4, data_long_fips$Year, sep="-")

data_long_final <- data_long_fips %>% 
  select(County, FIPS.x, Year, `Measure Type`, Variable.x, Value) 
...
data_long_final <- data_long_final %>% 
  arrange(County, Year, `Measure Type`, desc(Variable))

write.table(
    data_long_final,
    file.path("data", "median-home-value-county-2016.csv"),
    sep = ",",
    row.names = F,
    col.names = T,
    na = "-9999" 
